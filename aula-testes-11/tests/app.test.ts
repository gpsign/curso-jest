...
describe("POST /users tests", () => {
	...
	it("should receive 409 when trying to create two users with same e-mail", async () => {
		const { email, password } = await createUser("e@mail.com", "123456");

		const { status } = await api.post("/users").send({ email, password });
...
describe("GET /users tests", () => {
	it("should return a single user", async () => {
		const user = await createUser("e@mail.com", "123456");
	...
	it("should return all users", async () => {
		const user = await createUser("e@mail.com", "123456");
		const user2 = await createUser("e2@mail.com", "123456");
